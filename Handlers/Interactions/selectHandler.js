import { APIMessageComponentSelectMenuInteraction, MessageFlags, APIUser } from 'discord-api-types/v10';
    ...
    if ( CooldownStartTimestamp != undefined ) {
        ...
        if ( Now < ExpirationTime ) {
            ...
            if ( timeLeft >= 60 && timeLeft < 3600 ) {
                ...
                await api.interactions.reply(interaction.id, interaction.token, {
                    flags: MessageFlags.Ephemeral,
                    content: localize('en-GB', 'SELECT_ERROR_COOLDOWN_MINUTES', timeLeft.toFixed(1))
                });
            ...
            // HOURS
                ...
                await api.interactions.reply(interaction.id, interaction.token, {
                    flags: MessageFlags.Ephemeral,
                    content: localize('en-GB', 'SELECT_ERROR_COOLDOWN_HOURS', timeLeft.toFixed(1))
                });
                return 'COOLDOWN_ACTIVE';
            }
            // DAYS
                ...
                await api.interactions.reply(interaction.id, interaction.token, {
                    flags: MessageFlags.Ephemeral,
                    content: localize('en-GB', 'SELECT_ERROR_COOLDOWN_DAYS', timeLeft.toFixed(1))
                });
                return 'COOLDOWN_ACTIVE';
            }
            // MONTHS
                ...
                await api.interactions.reply(interaction.id, interaction.token, {
                    flags: MessageFlags.Ephemeral,
                    content: localize('en-GB', 'SELECT_ERROR_COOLDOWN_MONTHS', timeLeft.toFixed(1))
                });
                return 'COOLDOWN_ACTIVE';
            }
            // SECONDS
                ...
                await api.interactions.reply(interaction.id, interaction.token, {
                    flags: MessageFlags.Ephemeral,
                    content: localize('en-GB', 'SELECT_ERROR_COOLDOWN_SECONDS', timeLeft.toFixed(1))
                });
                return 'COOLDOWN_ACTIVE';
            }
        }
